...
def save_images(obstacle_files: List[str], centerobj_file : str = None) -> None:
    """Saves rendered images of the object in the scene."""
    os.makedirs(args.output_dir, exist_ok=True)
    list_folder = os.listdir(args.output_dir)
    list_folder = [f for f in list_folder if os.path.isdir(os.path.join(args.output_dir, f))]
    output_dir = os.path.join(args.output_dir, f"{len(list_folder)}")
    os.makedirs(output_dir, exist_ok=True)
    
    
    
    num_obs = len(obstacle_files)
    obss = []
    for idx, obstacle_f in enumerate(obstacle_files):
        trans = Vector([0.6 * np.cos(idx* np.pi*2 / num_obs), 0.6 * np.sin(idx * np.pi*2 / num_obs), 0])
        obs = bproc.loader.load_obj(obstacle_f)
        obss.append(obs)
        normalize_meshobj(obs, translation= trans)
    center_obj = bproc.loader.load_obj(centerobj_file)
    normalize_meshobj(center_obj)    
    ...
    with open(os.path.join(output_dir, "description.txt"), "w") as f:
        f.write("Obstacle\n")
        for obsf  in obstacle_files:
            f.write(f"{obsf}\n")
        f.write("Center\n")
        f.write(f"{centerobj_file}\n")


if __name__ == "__main__":
    try:
        start_i = time.time()
        save_images(args.obstacle_path, args.center_path)
        end_i = time.time()
        print("Finished", args.center_path, "in", end_i - start_i, "seconds")
    except Exception as e:
        print("Failed to render", args.center_path)
        print(e)